...
public class ItemAttributes implements Serializable
{
	...
	public Map<Element, Integer> getElements()
	{
		...
		if (fire > 0)
			elements.put(Element.FIRE, Integer.valueOf(fire));
		if (water > 0)
			elements.put(Element.WATER, Integer.valueOf(water));
		if (wind > 0)
			elements.put(Element.WIND, Integer.valueOf(wind));
		if (earth > 0)
			elements.put(Element.EARTH, Integer.valueOf(earth));
		if (holy > 0)
			elements.put(Element.HOLY, Integer.valueOf(holy));
		if (unholy > 0)
			elements.put(Element.UNHOLY, Integer.valueOf(unholy));
		return elements;
  	}
	public int getValue()
	{
