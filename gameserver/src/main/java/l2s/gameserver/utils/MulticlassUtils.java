...
public final class MulticlassUtils
{
		...
		if(cmd.equalsIgnoreCase("list"))
		{
			if(st.hasMoreTokens())
			{
				...
				if(st.hasMoreTokens())
				{
					...
					ClassType classType = ClassType.MAIN_TYPES[classTypeId];
	...
	private static void showMulticlassList(Player player, Race race, ClassType classType)
	{
		...
		TIntStringHashMap tpls = HtmCache.getInstance().getTemplates("custom/multiclass.htm", player);
		...
		{
			for(ClassType c : ClassType.MAIN_TYPES)
			{
				for(ClassId classId : ClassId.VALUES)
				{
					...
					if(classId.getType().getMainType() != c)
						continue;
		...
		else
		{
			for(ClassId classId : ClassId.VALUES)
			{
				...
				if(classType != null && showByClassType && classId.getType().getMainType() != classType)
					continue;
	...
	{
		final Collection<SkillLearn> skills = SkillAcquireHolder.getInstance().getAvailableSkills(player, classId, AcquireType.MULTICLASS, null);
