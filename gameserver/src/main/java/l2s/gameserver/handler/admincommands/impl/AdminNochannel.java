			...
			if(LastBanChatDayTime != 0)
			{
				if(System.currentTimeMillis() - LastBanChatDayTime < 1000 * 60 * 60 * 24)
				{
					if(banChatCount >= banChatCountPerDay)
					{
						if(activeChar.isLangRus())
							activeChar.sendMessage("В сутки, вы можете выдать не более " + banChatCount + " банов чата."); //TODO [G1ta0] custom message
						else
							activeChar.sendMessage("You can ban no more than " + banChatCount + " ban chats in one day.");							
						return false;
					}
				}
				else
				{
					...
					if(activeChar.getPlayerAccess().BanChatBonusId > 0 && activeChar.getPlayerAccess().BanChatBonusCount > 0)
					{
						...
						if(activeChar.isLangRus())
							activeChar.sendMessage("Бонус за модерирование: " + add_count + " " + item.getName()); //TODO [G1ta0] custom message
						else
							activeChar.sendMessage("Your bonus for moderating is: " + add_count + " " + item.getName());

						if(penaltyCount > 0) // У модератора был штраф за нарушения
						{
							if(activeChar.isLangRus())
								activeChar.sendMessage("Штраф за нарушения: " + penaltyCount + " " + item.getName()); //TODO [G1ta0] custom message
							else
								activeChar.sendMessage("Penalty for breaking the rules: " + penaltyCount + " " + item.getName());						
						...
						if(add_count > 0)
							ItemFunctions.addItem(activeChar, activeChar.getPlayerAccess().BanChatBonusId, add_count, true, "Admin ban chat bonus");
					}
		...
		switch(command)
		{
			...
			case admin_nc:
			{
				...
				if(banChatCountPerDay > -1 && msg.startsWith("Вы забанили чат"))
				{
					...
					if(activeChar.isLangRus())
						activeChar.sendMessage("У вас осталось " + (banChatCountPerDay - banChatCount) + " банов чата."); //TODO [G1ta0] custom message
					else
						activeChar.sendMessage("You have " + (banChatCountPerDay - banChatCount) + " ban chats left.");
				}
			}
		}
		return true;
