...
public class AdminSpawn implements IAdminCommandHandler
{
		...
		switch(command)
		{
			...
			case admin_setai:
				...
				if(aiConstructor != null)
				{
					try
					{
						target.setAI((NpcAI) aiConstructor.newInstance(new Object[] { target }));
					}
					catch(Exception e)
					{
						e.printStackTrace();
	...
	private void spawnMonster(Player activeChar, String monsterId, int respawnTime, int mobCount)
	{
		...
		SpawnsDAO.getInstance().insert(template.getNpcId(), loc.x, loc.y, loc.z, heading, respawnTime, mobCount);
	}
}