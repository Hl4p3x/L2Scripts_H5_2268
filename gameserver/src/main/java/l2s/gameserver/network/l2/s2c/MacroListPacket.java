...
public class MacroListPacket extends L2GameServerPacket
{
	...
	private final int _rev, _macroId, _count;
	private final Action _action;
	private final Macro _macro;

	public MacroListPacket(int rev, int macroId, Action action, int count, Macro macro)
	{
		_rev = rev;
		_macroId = macroId;
		_action = action;
	...
	protected final void writeImplHF()
	{
		...
		if(_macro != null)
		{
			...
			writeS(_macro.descr); //Desc
			writeS(_macro.acronym); //acronym
			writeC(Math.max(0, Math.min(6, _macro.icon))); //icon
