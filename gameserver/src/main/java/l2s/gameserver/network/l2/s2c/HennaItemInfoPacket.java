...
public class HennaItemInfoPacket extends L2GameServerPacket
{
	...
	protected final void writeImpl()
	{
		...
		writeH(_int + _henna.getStatINT()); //equip INT
		writeD(_str); //current STR
		writeH(_str + _henna.getStatSTR()); //equip STR
		writeD(_con); //current CON
		writeH(_con + _henna.getStatCON()); //equip CON
		writeD(_men); //current MEM
		writeH(_men + _henna.getStatMEN()); //equip MEM
		writeD(_dex); //current DEX
		writeH(_dex + _henna.getStatDEX()); //equip DEX
		writeD(_wit); //current WIT
		writeH(_wit + _henna.getStatWIT()); //equip WIT
