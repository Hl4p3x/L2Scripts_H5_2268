...
public class FreyaEvent extends Functions implements ScriptFile, OnDeathListener, OnPlayerEnterListener
{
	...
	public void receiveGift()
	{
		...
		{
			if(ItemFunctions.getItemCount(player, ADENA) >= GIFT_PRICE)
			{
				ItemFunctions.deleteItem(player, ADENA, GIFT_PRICE);
				ItemFunctions.addItem(player, GIFT_ID, 1, "Freya Event gift");
		...
		else
		{
			...
			else if(ItemFunctions.getItemCount(player, ADENA) >= GIFT_PRICE)
			{
				ItemFunctions.deleteItem(player, ADENA, GIFT_PRICE);
				ItemFunctions.addItem(player, GIFT_ID, 1, "Freya Event gift");
				player.setVar("FreyaCelebration", String.valueOf(_curr_time), -1);
