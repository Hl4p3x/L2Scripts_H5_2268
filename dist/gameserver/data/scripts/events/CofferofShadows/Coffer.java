...
public class Coffer extends ScriptItemHandler
{
	...
	public boolean useItem(Playable playable, ItemInstance item, boolean ctrl)
	{
		...
		if(item.getCount() > 0 && activeChar.isQuestContinuationPossible(false))
		{
			activeChar.getInventory().destroyItem(item, 1); //FIXME [G1ta0] item-API
		...
		activeChar.sendPacket(SystemMessagePacket.removeItems(item.getItemId(), 1));
