...
public class FightClubManager extends Functions implements ScriptFile, OnPlayerExitListener, OnTeleportListener
{
	...
	public static void teleportPlayersToColliseum(Player player1, Player player2, Reflection reflection)
	{
		...
		if(Config.CANCEL_BUFF_BEFORE_FIGHT)
		{
			...
			EffectsDAO.getInstance().deleteSummonsEffects(player1); // Удаляем все эффекты непризванных саммонов.
			if(player1.getServitor() != null)
				player1.getServitor().getEffectList().stopAllEffects();
		}
		...
		if(Config.CANCEL_BUFF_BEFORE_FIGHT)
		{
			...
			EffectsDAO.getInstance().deleteSummonsEffects(player2); // Удаляем все эффекты непризванных саммонов.
			if(player2.getServitor() != null)
				player2.getServitor().getEffectList().stopAllEffects();
		}
