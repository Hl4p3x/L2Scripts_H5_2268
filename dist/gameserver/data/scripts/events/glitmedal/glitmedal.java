...
public class glitmedal extends Functions implements ScriptFile, OnDeathListener, OnPlayerEnterListener
{
	...
	public void onDeath(Creature cha, Creature killer)
	{
		if(_active && SimpleCheckDrop(cha, killer))
		{
			...
			if(count > 0)
				ItemFunctions.addItem(killer.getPlayer(), EVENT_MEDAL, count, "Glit Medal drop");
			if(killer.getPlayer().getInventory().getCountOf(Badge_of_Wolf) == 0 && Rnd.chance(Config.EVENT_GLITTMEDAL_GLIT_CHANCE * killer.getPlayer().getRateItems() * ((MonsterInstance) cha).getTemplate().rateHp))
				ItemFunctions.addItem(killer.getPlayer(), EVENT_GLITTMEDAL, 1, "Glit Medal drop");
		}
	}
	...
	public void glitchang()
	{
		...
		if(ItemFunctions.getItemCount(player, EVENT_MEDAL) >= 1000)
		{
			...
			ItemFunctions.addItem(player, EVENT_GLITTMEDAL, 10, "Glit Medal exchange");
			return;
		}
	...
	public void gamea()
	{
		...
		if(ItemFunctions.getItemCount(player, Badge_of_Fox) >= 1)
		{
			if(ItemFunctions.getItemCount(player, EVENT_GLITTMEDAL) >= 40)
				if(isTalker == 1)
				{
					...
					ItemFunctions.addItem(player, Badge_of_Wolf, 1, "Glit Medal reward");
		...
		else if(ItemFunctions.getItemCount(player, Badge_of_Hyena) >= 1)
		{
			if(ItemFunctions.getItemCount(player, EVENT_GLITTMEDAL) >= 20)
				if(isTalker == 1)
				{
					...
					ItemFunctions.addItem(player, Badge_of_Fox, 1, "Glit Medal reward");
		...
		else if(ItemFunctions.getItemCount(player, Badge_of_Rabbit) >= 1)
		{
			if(ItemFunctions.getItemCount(player, EVENT_GLITTMEDAL) >= 10)
				if(isTalker == 1)
				{
					...
					ItemFunctions.addItem(player, Badge_of_Hyena, 1, "Glit Medal reward");
		...
		if(ItemFunctions.getItemCount(player, EVENT_GLITTMEDAL) >= 5)
			if(isTalker == 1)
			{
				...
				ItemFunctions.addItem(player, Badge_of_Rabbit, 1, "Glit Medal reward");
