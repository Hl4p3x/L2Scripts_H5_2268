...
public class LeylaMira extends DefaultAI
{
	...
	protected void onEvtSpawn()
	{
		...
		addTaskMove(new Location( -56657, -56338, -2006), true, false);
		doTask();
	...
	private class ScheduleStart implements Runnable
	{
		...
		public void run()
		{
			...
			{
				case 1:
					_actor.broadcastPacket(new PlaySoundPacket(PlaySoundPacket.Type.MUSIC, "NS22_F", 1, 0, _actor.getLoc()));
					ThreadPoolManager.getInstance().schedule(new ScheduleStart(2, _actor), 100);
					break;
				case 2:
					_actor.broadcastPacket(new SocialActionPacket(_actor.getObjectId(), 3));
				...
				case 3:
					...
					{
						count++;
						_actor.broadcastPacket(new SocialActionPacket(_actor.getObjectId(), 1));
					...
					break;
				case 4:
					_actor.broadcastPacket(new SocialActionPacket(_actor.getObjectId(), 2));
